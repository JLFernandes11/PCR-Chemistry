...
# PCR with graphical user interface
...
window = tk.Tk()
window.title('PCR Chemical analysis')
...
# Creating the PCR function
def pcr(X_centered, Y_centered, pc):
    ...
    Xreg = pca.fit_transform(X_centered)[:,:pc]
    ...
    #... Fit
    regr.fit(Xreg, Y_centered)
    
    #... Calibration
    y_c = regr.predict(Xreg)
    
    #... Cross-validation
    y_cv = cross_val_predict(regr, Xreg, Y_centered, cv=10)
...
for n_comp in xticks:
    predicted_pc, r2r_pc, r2cv_pc, mser_pc, mscv_pc = pcr(X_centered, Y_centered, pc=n_comp)
...
d1X = savgol_filter(X, 18, polyorder=1, deriv=1)

#... Second derivative to make the graph
d2X = savgol_filter(X, 18, polyorder=2, deriv=2) 
...
#... PCR performance with the desired number of components
predicted, r2r, r2cv, mser, mscv = pcr(X_centered, Y_centered, pc)
...
def pcr_sample(X_sam_cen, Y_centered, pc, X_centered, scaler_Y):
    ...
    Xreg = pca.fit_transform(X_centered)[:,:pc] # train PCA on standards
    ...
    # Reduce the spectral data of the sample with PCA with the selected number of pc
    Xreg_sample = pca.transform(X_sam_cen)[:,:pc] # (using the same loadings from the standards)

    # Getting the concentrations of the unknown sample
    y_s = regr.predict(Xreg_sample)

    # De-centering the concentration of unknown sample
    y_sample = scaler_Y.inverse_transform(y_s)

    # Printing the determined concentration of unkown sample in a csv file
